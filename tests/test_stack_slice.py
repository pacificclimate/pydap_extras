...
from pydap_extras.handlers.stack_slice import StackableSlice as ss


@pytest.mark.parametrize(
    ("slice_zero", "slice_one", "expected_output"),
    [
        (ss(None), ss(None), range(10)),
        (ss(None), ss(2, 8, 1), range(2, 8)),
        (ss(2, 8, 1), ss(0, 6, 2), range(2, 8, 2)),
        (ss(2, 8, 2), ss(0, 3, 1), range(2, 7, 2)),
        (ss(2, 8, 2), ss(0, 4, 2), range(2, 8, 4)),
    ],
)
