        ...
        def add_variables(dataset, h5, level=0):
            ...
            if rank == 0:
                dataset[name] = BaseType(
                    name, data=Hdf5Data(h5), dimensions=(), attributes=attrs
                )
            # sequence?
            # elif rank == 1:
...
def find_unlimited(h5):
    ...
    if type(h5) == h5py.Dataset:
        ...
        rv = [
            dimname for dimname, length in zip(dims, maxshape) if not length
...
def process_attrs(attrs):
    ...
    for key in attrs.keys():
        try:
            val = attrs.get(
                key
            )  # Potentially raises TypeError: No NumPy equivalent for TypeVlenID exists
            REVERSE(
                val.dtype
    ...
    def __getitem__(self, slices):
        ...
        for s in slices:
            if type(s) == StackableSlice:
                ...
                raise TypeError(
                    "__getitem__ should be called with a list of slices (or StackableSlices), not {}".format(
                        [type(s) for s in slices]
                    )
                )
        slices = converted_slices

        subset_slices = [
            orig_slice + subset_slice
            for orig_slice, subset_slice in zip(self._slices, slices)
        ]
...
def sliced_shape(slice_, shape_):
    ...
    rv = [
        sh if sl == slice(None) else len(range(sh)[sl])
