...
class AAIGridResponse(BaseResponse):
    ...
    def __init__(self, dataset):
        ...
        if not dataset:
            raise HTTPBadRequest(
                "The ArcASCII Grid (aaigrid) response did not receive required dataset parameter"
            )
        ...
        if not self.grids:
            raise HTTPBadRequest(
                "The ArcASCII Grid (aaigrid) response only supports GridTypes, yet none are included in the requested dataset: {}".format(
                    dataset
                )
            )
        ...
        for grid in self.grids:
            ...
            if l not in (2, 3):
                raise HTTPBadRequest(
                    "The ArcASCII Grid (aaigrid) response only supports Grids with 2 or 3 dimensions, but one of the requested grids contains {} dimension{}".format(
                        l, "s" if l > 1 else ""
                    )
                )
            try:
                ...
                raise HTTPBadRequest(
                    "The ArcASCII Grid (aaigrid) response could not detect the grid transform for grid {}: {}".format(
                        grid.name, e.message
                    )
                )
    ...
    def __iter__(self):
        ...
        def generate_aaigrid_files(grid):
            ...
            for file_ in _grid_array_to_gdal_files(
                grid.array, srs, geo_transform, filename_fmt=output_fmt, missval=missval
            ):
                yield file_
...
def _grid_array_to_gdal_files(
    dap_grid_array, srs, geo_transform, filename_fmt="{i}.asc", missval=None
):
    ...
    logger.debug(
        "_grid_array_to_gdal_files: translating this grid {} of this srs {} transform {} to this file {}".format(
            dap_grid_array, srs, geo_transform, filename_fmt
        )
    )
    ...
    if len(shp) == 2:
        ...
        raise ValueError(
            "_grid_array_to_gdal_files received a grid of rank {} rather than the required 2 or 3".format(
                len(shp)
            )
        )
    ...
    meta_ds = create_gdal_mem_dataset(
        xlen, ylen, geo_transform, srs, target_type, missval
    )
    ...
    for i, layer in enumerate(data):
        ...
        if len(layer.shape) == 3 and layer.shape[0] == 1:
            ...
            raise ValueError(
                "_grid_array_to_gdal_files received a layer of shape {}".format(
                    layer.shape
                )
            )
...
def create_gdal_mem_dataset(xlen, ylen, geo_transform, srs, target_type, missval=None):
    ...
    """
    logger.debug(
        "Creating a GDAL driver ({}, {}) of type {}".format(xlen, ylen, target_type)
    )
...
def get_time_map(dst):
    ...
    for map_name, map_ in dst.maps.items():
        ...
        if "standard_name" in attrs and attrs["standard_name"] == "time":
            return map_
        if "units" in attrs and re.match(
            "(day|d|hour|h|hr|minute|min|second|sec|s)s? since .+", attrs["units"]
        ):
            return map_
    return None
...
def detect_dataset_transform(dst):
    ...
    if type(xmap.data) == numpy.ndarray:
        ...
        xarray = next(
            iter(xmap.data)
        )  # Might to iterate over proxy objects to actually get the data
